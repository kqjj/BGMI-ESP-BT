	...
	case WM_SIZE:
		if (DX11::pDevice != NULL && wParam != SIZE_MINIMIZED)
		{
			...
			if (DX11::pSwapChain)
			{
				...
				//texture getting worse
				// hr = DX11::pSwapChain->ResizeBuffers(0, GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN), DXGI_FORMAT_UNKNOWN, 0);
				...
				// Check if the target window is in fullscreen mode
				//bool isFullscreen = (Rect.left == 0 && Rect.top == 0 &&
				//	Rect.right == GetSystemMetrics(SM_CXSCREEN) &&
				//	Rect.bottom == GetSystemMetrics(SM_CYSCREEN));
				//
				//// Set fullscreen state of the swap chain accordingly
				//DX11::pSwapChain->SetFullscreenState(isFullscreen, NULL);
