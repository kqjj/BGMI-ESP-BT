...
VOID DX11::ImGui_DX11::DrawBorderStringOpt(ImFont* pFont, FLOAT FontSize, string& Text, FLOAT X, FLOAT Y, ImColor Color, bool Center) {
	pImGuiDraw = ImGui::GetBackgroundDrawList();
	...
	for (const auto& pos : offsetPos) {
		pImGuiDraw->AddText(pFont, FontSize, pos, DX11::ImGui_DX11::Color::Black, Text.c_str());
	}

	pImGuiDraw->AddText(pFont, FontSize, TextPos, Color, Text.c_str());
}
